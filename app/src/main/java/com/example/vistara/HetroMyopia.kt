...
import com.example.vistara.databinding.ActivityHetroMyopiaBinding
import java.util.*

class HetroMyopia : AppCompatActivity(), TextToSpeech.OnInitListener {
    private lateinit var binding: ActivityHetroMyopiaBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_hetro_myopia)
    ...
    private fun setupUI() {
        binding.progressBar.max = snellenChart.size * 10
        binding.btnSpeak.setOnClickListener { speakCurrentRow() }
        binding.btnSubmit.setOnClickListener { checkAnswer() }
        binding.btnSkip.setOnClickListener { skipToNext() }
        binding.btnRestart.setOnClickListener { restartTest() }
    ...
    private fun finishTest() {
        ...
        val message = when {
            percentage >= 80 -> "Excellent vision! No signs of hetromyopia detected."
            percentage >= 50 -> "Mild hetromyopia may be present. Consider an eye exam."
            else -> "Significant hetromyopia detected. Please consult an optometrist."
        }
